package fr.ubx.poo.ubgarden.game;

import fr.ubx.poo.ubgarden.game.go.bonus.Carrots;
import fr.ubx.poo.ubgarden.game.go.bonus.EnergyBoost;
import fr.ubx.poo.ubgarden.game.go.bonus.Insecticide;
import fr.ubx.poo.ubgarden.game.go.bonus.PoisonedApple;
import fr.ubx.poo.ubgarden.game.go.decor.*;
import fr.ubx.poo.ubgarden.game.go.decor.Door;
import fr.ubx.poo.ubgarden.game.go.decor.ground.Grass;
import fr.ubx.poo.ubgarden.game.go.decor.ground.Land;
import fr.ubx.poo.ubgarden.game.launcher.MapEntity;
import fr.ubx.poo.ubgarden.game.launcher.MapLevel;

import java.util.ArrayList;
import java.util.Collection;
import java.util.HashMap;
import java.util.List;

public class Level implements Map {

    private final int level;
    private final int width;

    private final int height;

    private final java.util.Map<Position, Decor> decors = new HashMap<>();
    private final List<Carrots> carrotsList = new ArrayList<>();

    public Level(Game game, int level, MapLevel entities) {
        this.level = level;
        this.width = entities.width();
        this.height = entities.height();

        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++) {
                Position position = new Position(level, i, j);
                MapEntity mapEntity = entities.get(i, j);
                switch (mapEntity) {
                    case Grass:
                        decors.put(position, new Grass(position));
                        break;
                    case Land:
                        decors.put(position, new Land(position));
                        break;

                    case Tree:
                        decors.put(position, new Tree(position));
                        break;
                    case Hedgehog:
                        Hedgehog hedgehog = new Hedgehog(position);
                        decors.put(position, hedgehog);
                        game.setHedgehog(hedgehog);
                        break;
                    case Flowers:
                        decors.put(position, new Flowers(position));
                        break;
                    case NestWasp:
                        NestWasp nestWasp = new NestWasp(game, position);
                        decors.put(position, nestWasp);
                        break;
                    case NestHornet:
                        NestHornet nestHornet = new NestHornet(game, position);
                        decors.put(position, nestHornet);
                        break;
                    case Carrots: {
                        Decor land = new Land(position);
                        Carrots carrots = new Carrots(position, land);
                        land.setBonus(carrots);
                        decors.put(position, land);
                        carrotsList.add(carrots);
                        break;
                    }
                    case Apple: {
                        Decor grass = new Grass(position);
                        grass.setBonus(new EnergyBoost(position, grass));
                        decors.put(position, grass);
                        break;
                    }
                    case DoorNextClosed: {
                        Door door = new Door(position, level + 1, false);
                        decors.put(position, door);
                        break;
                    }
                    case DoorNextOpened: {
                        Door door = new Door(position, level + 1, true);
                        decors.put(position, door);
                        break;
                    }
                    case DoorPrevOpened: {
                        Door door = new Door(position, level - 1, true);
                        decors.put(position, door);
                        break;
                    }
                    case PoisonedApple: {
                        Decor grass = new Grass(position);
                        grass.setBonus(new PoisonedApple(position, grass));
                        decors.put(position, grass);
                        break;
                    }

                    case Insecticide: {
                        Decor grass = new Grass(position);
                        grass.setBonus(new Insecticide(position, grass));
                        decors.put(position, grass);
                        break;
                    }
                    default:
                        throw new RuntimeException("EntityCode " + mapEntity.name() + " not processed");
                }
            }
    }

    @Override
    public int width() {
        return this.width;
    }

    @Override
    public int height() {
        return this.height;
    }

    public Decor get(Position position) {
        return decors.get(position);
    }

    public Collection<Decor> values() {
        return decors.values();
    }


    @Override
    public boolean inside(Position position) {
        return position.level() == level && position.x() >= 0 && position.x() < width && position.y() >= 0 && position.y() < height;
    }
    public boolean allCarrotsCollected() {
        for (Carrots carrots : carrotsList) {
            if (!carrots.isPickedUp()) {
                return false;
            }
        }
        return true;
    }

        public void openAllDoors() {
            for (Decor decor : decors.values()) {
                if (decor instanceof Door) {
                    Door door = (Door) decor;
                    if (!door.isOpen()) {
                        door.setOpen(true);
                    }
                }
            }
        }

}
